...
Homology
: Refers to genes (or proteins), that are descended from a gene (or protein) present in a common ancestor. 

Orthology
: Genes or proteins separated (that have diverged) by speciaciation events are called orthologs.
...
![Figure 4. Protein tree of the GS family from the conifer species shown in the previous figure. The phylogenetic relationship in conflict with the species tree has been emphasize by pink rectangles. ](./Figures/conif_ptr.png){width=50%}
&nbsp;
...
If all we want is to know whether or not all GS1a are orthologs among them, and which GS1a proteins are orthologs (diverge by speciation) and which one are paralogs (diverge by duplication), we can make it easy by running the function `orthG()`, which takes as argument the set of species of interest, in our case the 12 conifer species and returns two object. The first one is an adjancency matrix. Since in our case the 12 species contribute 29 GS proteins, we expect a 29 x 29 square matrix (entries with 1 meaning orthology, and entries with 0 meaning paralogy). The second object returned by `orthG()` is a network graph. Two nodes (GS proteins) are conected when they are orthologs. Let's see this function in action!

```{r, eval=FALSE, include=TRUE}
o <- orthG(set = c("Pa","Psm","Pp","Pin","Abi","Ap",
              "Ara","Pod","Nag","Sci","Tba","Tax"))
plot(o[[2]])
...
```{r}
p <- paste(system.file("extdata", package = "orthGS"), "/conifers", sep  = "")
m <- mapTrees(path2rec = p) 
# plot(m[[1]], cex = 0.5)
# ape::nodelabels(m[[1]]$node,frame = "none", cex = 0.6)
# orthG(set = conif$short)
# orthP("Pp_GS1a", set = "all")
```
...
9. Menet H, Daubin V, Tannier E. Phylogenetic reconciliation. *PLoS Comput Biol* 2022, *18*(11): e1010621. https://doi.org/10.1371/journal.
pcbi.1010621.
